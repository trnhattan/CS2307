...
VALUES
    ...
    ('LLM_PROVIDER', '"openrouter"'::JSONB,
     'LLM provider selected by the administrator: openrouter or gemini.', TRUE),
    ('LLM_MODEL', '"~deepseek/deepseek-v4-flash-latest"'::JSONB,
     'Configured LLM model. A gemini- model uses Gemini; other model names use OpenRouter.', TRUE),
    ('LLM_REASONING_ENABLED', 'true'::JSONB,
     'Request provider-supported reasoning while keeping internal details off the learner UI.', TRUE),
